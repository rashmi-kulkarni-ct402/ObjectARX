// Exercise8_SelectObjectsAndRequestKeyword.cpp : Defines the initialization routines for the DLL.
	...
	if (acedSSGet(NULL, NULL, NULL, NULL, sset) == RTNORM)
	{
		...
		if (retVal == RTNORM || retVal == RTNONE)
		{
			...
			for (int loopCnt = 0; loopCnt < lSSCnt; loopCnt++)
			{
				...
				// Get the object id for the object from the selection set
				AcDbObjectId objId;
				...
				// Change the object's color based on the keyword entered
				AcCmColor color;
